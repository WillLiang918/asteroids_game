(function () {
  ...
  EnemyShip.prototype.move = function (delta) {
    ...
    if (dist > 300) {
      this.vel[0] += Math.sin(this.angle) * (this.game.speed / 20);
      this.vel[1] -= Math.cos(this.angle) * (this.game.speed / 20);
    }
  ...
  EnemyShip.prototype.relocate = function () {
    ...
    this.vel = [0, 0];
    this.health = 100;
    if ( this.game.score > 1250 && this.game.enemyShips.length < 3) {
