(function () {
  ...
  var FleetShip = Asteroids.FleetShip = function (options) {
    var target_idx = options.game.enemyShips.length - 1;
    options.targetShip = options.game.enemyShips[target_idx];
    console.log(options.targetShip);
    options.health = 50;
  ...
  FleetShip.prototype.collideWith = function (otherObject) {
  ...
  FleetShip.prototype.relocate = function () {
    ...
    if (
      ...
      this.game.enemyShips.length > 3 &&
      this.game.fleetShips.length < 4
    ) {
        this.game.addFleetShip();
    } else if ( this.game.score > 1000 &&
      this.game.enemyShips.length > 2 &&
      this.game.fleetShips.length < 3
    ) {
      this.game.addFleetShip();
    } else if ( this.game.score > 500 &&
      this.game.enemyShips.length > 1 &&
      this.game.fleetShips.length < 2
    ) {
      this.game.addFleetShip();
    }
  };
